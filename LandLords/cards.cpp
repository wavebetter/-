...
void Cards::add(const Cards& cards){
    m_cards.unite(cards.m_cards);//两个集合合并成一个集合
}
...
void Cards::remove(const Cards& cards){
    m_cards.subtract(cards.m_cards);//相当于做一个差集
}
...
Card Cards::takeRandCard(){
    ...
    Card card=*it;
    m_cards.erase(it);//remove 方法接受一个值作为参数，并删除所有等于该值的元素,erase 方法接受一个迭代器作为参数，并删除该迭代器所指向的元素
    return card;
}
CardList Cards::toCardList(SortType type){
    ...
    if(type==Asc){
        ...
        std::sort(list.begin(),list.end(),greatorSort);
    }
    return list;
}
